...
(function () {
  ...
  const state = {
    regards: [
      { nom: 'R1', PM: 0, TN: 100.0, DN: 300, DE: 345, Q: 30, pente: 0.005, K: 80 },
      { nom: 'R2', PM: 50, TN: 99.5, DN: 300, DE: 345, Q: 45, pente: 0.004, K: 80 },
      { nom: 'R3', PM: 100, TN: 99.2, DN: 400, DE: 450, Q: 60, pente: 0.004, K: 80 },
      { nom: 'R4', PM: 160, TN: 98.7 }
    ]
  };
  ...
  function render() {
    ...
    c.appendChild(D.el('div', { class: 'rowline wide', style: 'font-size:0.72rem;color:#61708a;font-weight:600' }, [
      ...
      D.el('span', { style: 'width:64px' }, ['pente']), D.el('span', { style: 'width:64px' }, ['K']), D.el('span', {}, [''])
    ]));
  ...
  function compute() {
    const noeuds = state.regards.map((r) => ({ nom: r.nom, PM: r.PM, TN: r.TN }));
    const troncons = state.regards.slice(0, -1).map((r) => ({ DN: r.DN, DE: r.DE, K: r.K, Q: r.Q, pente: r.pente }));
    let res;
    try { res = GC.network.profil({ noeuds, troncons, filEauDepart: D.num('nw_fileau'), eLit: D.num('nw_elit') }); }
    catch (e) { D.$('#nw_res').innerHTML = `<p class="error">${e.message}</p>`; return; }

    let html = `<div class="result-head">${D.badge(res.statut)}<h4>Cotes des regards</h4></div>`;
    html += D.table(['Regard', 'PM', 'TN', 'Fil d’eau', 'Gén. sup.', 'Fond fouille', 'Couv.', 'H fouille'],
      res.noeuds.map((n) => [n.nom, D.fmt(n.PM, 0), D.fmt(n.TN, 2), D.fmt(n.filEau, 2), D.fmt(n.crown, 2),
    ...
    GC.plot.profilLong(D.$('#nw_plot'), res.noeuds);
  }
  ...
  function init() {
    ...
    D.$('#nw_add').addEventListener('click', () => {
      ...
      state.regards.push({ nom: 'R' + (state.regards.length + 1), PM: last.PM + 50, TN: last.TN - 0.3 });
      render();
    });
    D.$('#nw_calc').addEventListener('click', compute);
    compute();
  }
